...
const disemvowel2 = str => {
	...
	return arr.reduce((acc, e) => {
		if (!vowels[e.toLowerCase()]) return (acc += e);
		return acc;
	}, "");
};
...
const getLocations = arr => arr.map(e => e.location);
...
function runTests(questionNum, testCases, testCallback) {
	...
	try {
		for (let testCase of testCases) {
			...
			assert.strictEqual(
				JSON.stringify(testCallback(testCase.input)),
		...
		if (error.expected === undefined) {
			...
			console.log(
				`\nTest failed.  Was expecting "${error.expected}", but got "${
					error.actual
				}"`,
			);
		}
...
function runQ6Tests() {
	let testCases = [
		new TestCase(["a", "bb", "ccc", "dddd", "eeeee"], ["a", "ccc", "eeeee"]),
		new TestCase(
		...
		new TestCase(["one", "two", "three", "four"], ["one", "two", "three"]),
		new TestCase([], []),
		new TestCase(["a"], ["a"]),
		new TestCase(["to"], []),
	];
...
function runQ7Tests() {
	let testCases = [
		new TestCase(
			[new Day(50, "raining"), new Day(99, "sunny"), new Day(24, "snowing")],
			[
		...
		new TestCase(
			[new Day(31, "sleeting"), new Day(88, "partly cloudy")],
			["It is 31 degrees and sleeting", "It is 88 degrees and partly cloudy"],
		),
		new TestCase([], []),
	];
